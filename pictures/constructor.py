...
def unsplash_photos(
    ...
    UnsplashPhotos.objects.create(
        photo_id=photo_id,
        photo_url=photo_url,
        photo_image_url=photo_image_url,
        photo_submitted_at=photo_submitted_at,
        ...
        photo_location_name=photo_location_name,
        photo_location_country=photo_location_country,
        photo_location_city=photo_location_city,
        ai_description=ai_description,
        ai_primary_landmark_name=ai_primary_landmark_name,
    )
...
def unsplash_collections(photo_id, title, collected_at):
    ...
    UnsplashCollections.objects.create(
        photo=p, collection_title=title, photo_collected_at=collected_at
    )
...
def create_unsplash_photos():
    ...
    for row in zip(
        ...
        df["photo_url"],
        df["photo_image_url"],
        df["photo_submitted_at"],
        df["photo_featured"],
        df["photo_description"],
        df["photographer_username"],
        df["photographer_first_name"],
        df["photographer_last_name"],
        df["photo_location_name"],
        df["photo_location_country"],
        df["photo_location_city"],
        df["ai_description"],
        df["ai_primary_landmark_name"],
    ):
        unsplash_photos(
            photo_id=row[0],
            photo_url=row[1],
            photo_image_url=row[2],
            photo_submitted_at=row[3],
            photo_featured=row[4],
            photo_description=row[5],
            photographer_username=row[6],
            photographer_first_name=row[7],
            photographer_last_name=row[8],
            photo_location_name=row[9],
            photo_location_country=row[10],
            photo_location_city=row[11],
            ai_description=row[12],
            ai_primary_landmark_name=row[13],
        )
...
def create_random_images():
    ...
    for _ in range(10000):
        ...
        Images.objects.create(
            author=Profile.objects.get(id=random_id),
            unsplash_photo=UnsplashPhotos.objects.get(photo_id=random_photo),
        )
    return "success"
...
def rand_likes():
    ids = [u.id for u in Profile.objects.all()]
    for i in Images.objects.all():
        random_profile_count = random.randint(10, 30)
