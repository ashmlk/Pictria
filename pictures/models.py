...
hashedId_image_class = Hashids(salt='evero OIBOB80h0h obsdsdSFBV030 OOS9jfsv',min_length=12)
        ...
        qs = (
            ...
            .annotate(
                trigram = Greatest(
                    ...
                    (
                        Q(is_active=True)) & # only return active users 
                        (
                    ...
                    trigram__gte=0.03
                ).exclude(is_superuser=True).order_by('trigram') # do show superusers (admins) on search
            )
        return qs
...
class ImagesManager(models.Manager):
    ...
    def search(self, search_text):
        ...
        relevent_tags = Tag.objects.annotate(
            trigram=TrigramSimilarity('name',search_text)
        ).filter(
            trigram__gte=0.7
        ).order_by('-trigram')
        ...
        qs = (
            ...
            .annotate(
                ...
                trigram_author_username=TrigramSimilarity('author__username', search_text),
                trigram_author_last_name=TrigramSimilarity('author__last_name', search_text),
                trigram_author_first_name=TrigramSimilarity('author__first_name', search_text)        
            )
            .filter(
                (
                    Q(sv=search_query)|
    ...
    def get_top_tags(self):
        ...
        qs = self.get_queryset().filter(parent_image=None,last_edited__gte=time_threshold) 
    ...
    def get_tag_images(self,tag):
        ...
        qs = self.get_queryset().select_related("author").filter(tags=tag).order_by('-last_edited')
        if qs != None:
...
class Profile(AbstractUser):
    ...
    profile_image = models.ImageField(upload_to=upload_to_uuid('profile_image/'), blank=True)
    save_searches = models.BooleanField(default=True, blank=True) # boolean field to specify is user wants search results to be saved or not
    ...
    def get_image_url(self):   
        if not bool(self.profile_image):
            link = "https://avatars.dicebear.com/api/avataaars/" + self.get_hashed_id() +'.svg?' # get user avatar if user image is null (DiceBear API)
        else:
            link = "{0}".format(self.profile_image.url) # MEDIA_URL  can change based on production or development server
        return link
...
class SearchLog(models.Model):
    profile = models.ForeignKey(Profile, on_delete=models.CASCADE, related_name='searches', null = True)
...
class UnsplashPhotos(models.Model):
    ...
    photo_id = models.CharField(max_length=255,unique=True, null=False, primary_key=True)
...
class UnsplashCollections(models.Model):
    photo = models.ForeignKey(UnsplashPhotos, on_delete=models.CASCADE, related_name='collections')
...
class Images(models.Model):
    ...
    parent_image = models.ForeignKey('self', on_delete=models.CASCADE, related_name='child_images', null=True, blank=True)
    ...
    image_photo = models.ImageField(upload_to=upload_to_uuid('users_images/'), blank=True)
    #this is for demo purposes inly, remove ForeignKey relationship with UnsplashPhoto on real production
    unsplash_photo = models.ForeignKey(UnsplashPhotos, on_delete=models.CASCADE, null=True, related_name='unsplash_image')
    sv = pg_search.SearchVectorField(null=True)
    ...
    class Meta:
        indexes = [
            GinIndex(fields=['sv'],name='search_idx_images'),
        ]
    
    def save(self, *args, **kwargs):
        if self.pk is None: # image compression is done to avoid overloading server with large images
            try:
                MAX_WIDTH = 1080
                MAX_HEIGHT = 1350
                ...
                output = io.BytesIO()
                if exif:
                    img.save(output, format='JPEG', exif=exif, quality=80) # images will all be converted to JPEG  keeping quality of 80
                else:
                    img.save(output, format='JPEG', quality=80)
                output.seek(0)
                self.image_photo = InMemoryUploadedFile(output, 'ImageField', "%s.jpg" % self.image_photo.name, 'image/jpeg', output.getbuffer().nbytes, None)
    ...
    def get_unsplash_collection_title(self):
        if self.unsplash_photo != None:
            ...
            if collection_objs != None:
                return [c.collection_title.strip().lower() for c in collection_objs][:10]
            return None
...
class Bookmark(models.Model):
    ...
    image = models.ForeignKey(Images, on_delete=models.CASCADE, related_name='bookmarked')
    timestamp =  models.DateTimeField(auto_now_add=True)
